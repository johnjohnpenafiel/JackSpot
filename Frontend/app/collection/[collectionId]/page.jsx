...
function CollectionPage() {
  ...
  const { collections, fetchCollections, addSpot, deleteSpot } =
    useCollections();
  ...
  useEffect(() => {
    const fetchedCollection = collections.find(
  ...
  const sortedSpots = collection.spots
    .slice()
    .sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
  const spotList = sortedSpots.map((spot) => (
    <SpotItem
      key={spot.id}
      name={spot.name}
      type={spot.type}
      id={spot.id}
      deleteSpot={deleteSpotFromCollection}
    />
  ));
      ...
      <Header>
        <div className="inline-flex items-center justify-between gap-x-2">
          ...
          <FaPlus
            ...
            size={50}
            className="text-neutral-500/50 cursor-pointer mt-7 ml-2 hover:text-neutral-300/80 hover:scale-110 transition"
          />
        </div>
        <div className="items-center gap-x-2">
          <h3 className="text-neutral-700 semi-bold text-xl mt-1 flex flex-col px-2">
            Collection
          </h3>
        </div>
      </Header>
      <div className="md:flex flex-col gap-y-2 w-[500px] h-[655px] ml-5 p-2">
        <Box className="overflow-y-auto bg-neutral-500/10 px-2 py-3">
          <div className="flex flex-col gap-y-2 px-3 text-2xl">
            {spotList.length < 1 ? (
              <h1 className="text-neutral-500">This Collection is empty</h1>
            ) : (
              spotList
            )}
          </div>
